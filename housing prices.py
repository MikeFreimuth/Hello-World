'''
This builds a random forest and uses it to predict housing prices using data from Kaggle.
I made these all binary trees because it was easier to build that way.  When I did this I
had never really constructed something like a graph or a data structure from scratch before.
(Incidentally, I was super proud of this!)Anyway, all the splits for non-numerical features
are of the form "Is the value of the feature X or not X?"  It would probably be better if I
made it to handle splits into an arbitrary number of categories. This would take a serious
rebuild to change though I think.  I also probably could have been better at choosing variable
names in some places (some of those 'n's and 'm's for instance)....
'''
...
def build_tree(X,Y,n,m):    #n=maximum depth, m=number of variables to try for each split
    ...
    Y=np.array(Y)
    root=Node(X,Y)
    if n!=0 and len(X)>=6:   #Note: There is a hidden parameter here, it is the minimum number of observations
        a,b=split(X,Y,m)     #to keep splitting (currently 6).
        root.var=a
        root.split=b
        xleft=[]
        xright=[]
...
'''
Run to evaluate test data (check on X)

allX=X[:,1:-1]
allX=np.delete(allX,-4,1)
allY=X[:,-1]
forest=forest(allX,allY,512,8,7,200)

test=pd.read_csv(r'C:\\Users\\walru\\OneDrive\\Documents\\python\\Kaggle Housing Prices\\test.csv')
test=np.delete(test,0,1)
test=test.values
Yhat=evaluate(test, forest)

df=pd.DataFrame(yhat)
df.to_csv(r'C:\\Users\\walru\\OneDrive\\Documents\\python\\Kaggle Housing Prices\\predictions.csv')
'''
